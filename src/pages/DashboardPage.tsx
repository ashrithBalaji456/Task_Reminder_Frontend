...
export const DashboardPage: React.FC = () => {
  ...
  return (
    <Layout>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        {isLoading ? (
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Today's Completed</span>
              ...
              <p className="text-xs text-emerald-600 font-semibold mt-1 flex items-center gap-1">
                <span>{dashboardData?.completionPercentage || 0}% finished today</span>
                <ArrowRight className="w-3 h-3 group-hover:translate-x-1 transition-transform" />
              </p>
            </GlassCard>

            {/* Today's Pending Tasks Card */}
            <GlassCard
              onClick={() => navigate('/tasks')}
              className="cursor-pointer hover:border-amber-300 transition-all group"
            >
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Today's Pending</span>
            ...
            {/* Today's Completion Percentage Meter */}
            <GlassCard
              onClick={() => navigate('/analytics')}
              className="cursor-pointer hover:border-rose-300 transition-all group"
            >
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Today's Rate</span>
      ...
      {/* All-Time Database Overview Banner */}
      {!isLoading && dashboardData && (
        <GlassCard className="mb-8 border-l-4 border-l-purple-500 bg-gradient-to-r from-purple-50/60 to-pink-50/60">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <div className="flex items-center gap-2">
                <span className="font-extrabold text-slate-800 text-sm">📊 All-Time Database Stats</span>
                <span className="text-[10px] font-bold text-purple-700 bg-purple-100 px-2 py-0.5 rounded-full">Database Records</span>
              </div>
              <p className="text-xs text-slate-600 mt-1">
                Total Recorded: <strong className="text-slate-800">{dashboardData.allTimeTotalTasks ?? dashboardData.totalTasks}</strong> &nbsp;|&nbsp;
                Completed: <strong className="text-emerald-700">{dashboardData.allTimeCompletedTasks ?? dashboardData.completedTasks}</strong> &nbsp;|&nbsp;
                Pending: <strong className="text-amber-700">{dashboardData.allTimePendingTasks ?? dashboardData.pendingTasks}</strong> &nbsp;|&nbsp;
                Cancelled: <strong className="text-rose-700">{dashboardData.allTimeCancelledTasks ?? dashboardData.cancelledTasks}</strong>
              </p>
            </div>
            <AnimatedButton
              size="sm"
